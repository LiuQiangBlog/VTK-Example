/*=========================================================================
  ...
  Module:    vtkCornerChineseAnnotation.cpp
...
/**********************************************************************
  ...
  文件名: vtkCornerChineseAnnotation.cpp
...
#include "vtkCornerChineseAnnotation.h"
...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkCornerChineseAnnotation);

vtkSetObjectImplementationMacro(vtkCornerChineseAnnotation,ImageActor,vtkImageActor);
vtkSetObjectImplementationMacro(vtkCornerChineseAnnotation,WindowLevel,
                                vtkImageMapToWindowLevelColors);
vtkCxxSetObjectMacro(vtkCornerChineseAnnotation,TextProperty,vtkTextUnicodeProperty);

//----------------------------------------------------------------------------
vtkCornerChineseAnnotation::vtkCornerChineseAnnotation()
{
  ...
  this->TextProperty = vtkTextUnicodeProperty::New();
  this->TextProperty->ShadowOff();

  for (int i = 0; i < 4; i++)
    {
    this->CornerText[i] = NULL;
    this->TextMapper[i] = vtkFTGLChineseCharacterMapper::New();
...
void vtkCornerChineseAnnotation::ReleaseGraphicsResources(vtkWindow *win)
{
...
//----------------------------------------------------------------------------
void vtkCornerChineseAnnotation::TextReplace(vtkImageActor *ia,
...
//----------------------------------------------------------------------------
int vtkCornerChineseAnnotation::RenderOverlay(vtkViewport *viewport)
{
...
//----------------------------------------------------------------------------
int vtkCornerChineseAnnotation::RenderOpaqueGeometry(vtkViewport *viewport)
{
  int fontSize;
  int i;
  ...
  if (viewport_size_has_changed ||
    ...
    if (viewport_size_has_changed ||
      ...
      if (tprop_has_changed)
        {
        vtkTextUnicodeProperty *tprop = this->TextMapper[0]->GetTextProperty();
...
int vtkCornerChineseAnnotation::HasTranslucentPolygonalGeometry()
{
  return 0;
}

//----------------------------------------------------------------------------
void vtkCornerChineseAnnotation::SetTextActorsPosition(int vsize[2])
{
...
//----------------------------------------------------------------------------
void vtkCornerChineseAnnotation::SetTextActorsJustification()
{
  vtkTextUnicodeProperty *tprop = this->TextMapper[0]->GetTextProperty();
...
//----------------------------------------------------------------------------
void vtkCornerChineseAnnotation::SetText(int i, const char *text)
{
  if (i < 0 || i > 3)
    {
    return;
...
//----------------------------------------------------------------------------
const char* vtkCornerChineseAnnotation::GetText(int i)
{
  if (i < 0 || i > 3)
    {
    return NULL;
...
//----------------------------------------------------------------------------
void vtkCornerChineseAnnotation::ClearAllTexts()
{
  for (int i = 0; i < 4; i++)
    {
    this->SetText(i, "");
...
//----------------------------------------------------------------------------
void vtkCornerChineseAnnotation::CopyAllTextsFrom(vtkCornerChineseAnnotation *ca)
{
  for (int i = 0; i < 4; i++)
    {
...
//----------------------------------------------------------------------------
void vtkCornerChineseAnnotation::PrintSelf(ostream& os, vtkIndent indent)
{

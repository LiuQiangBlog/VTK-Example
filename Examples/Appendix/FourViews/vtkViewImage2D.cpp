...
void vtkViewImage2D::UpdatePosition ()
{
	...
	os << /*"Image size: "*/"图像大小: " << dims[xaxis] << " x " << dims[yaxis] << "\n";
	os << /*"Voxel size: "*/"像素大小: " << spacing[xaxis] << " x " << spacing[yaxis] << "\n";
	os << "X: " << imCoor[xaxis] << " px Y: " << imCoor[yaxis] << /*" px Value: "*/" px 像素值: "
		<< this->GetCurrentPointDoubleValue() << std::endl;
	os << "X: " << pos[xaxis] << " mm Y: " << pos[yaxis] << " mm\n";

	os2 << /*"Zoom: "*/"放大倍数: " << this->GetZoom()*100.0 << " %\n";
	if ( this->ShowSliceNumber )
		os2 << /*"Slice: "*/ "切片: "<< imCoor[orthogonalAxis] << " / " << dims[orthogonalAxis]-1 << std::endl;
	os2 << /*"Location: " */ "位置: " << pos[orthogonalAxis] << " mm";
...
void vtkViewImage2D::SetZoom (double factor)
{
	...
	if( this->GetImage() )
	{
		...
		int* dims = this->GetImage()->GetDimensions();
		std::ostringstream os;
		os << /*"Zoom: "*/"放大倍数: " << this->GetZoom()*100.0 << " %\n";
		int orthogonalAxis = this->GetOrthogonalAxis (this->Orientation);
		if ( this->ShowSliceNumber )
			os << /*"Slice: "*/"切片: " << imCoor[orthogonalAxis] << " / " << dims[orthogonalAxis]-1 << std::endl;
		os << /*"Location: "*/"位置: " << pos[orthogonalAxis] << " mm";

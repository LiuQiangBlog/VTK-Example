...
vtkFTGLChineseCharacterMapper::vtkFTGLChineseCharacterMapper()
{
	...
	this->m_TextUnicodeProperty = vtkTextUnicodeProperty::New();

	// 注意：由于在vtkCornerChineseAnnotation中做vtkTextUnicodeProperty对象的ShallowCopy时出错，
	// 所以这里将默认字体大小设置为10号，这是一个Bug，原则是不应该这么设置！
	this->m_TextUnicodeProperty->SetFontSize(10);
